...
func Match_Locations() (int, error) {
  ...
  for _, Symble_Info := range Symble_Arr {
    ...
    for _, Number_Info := range Number_Arr {
      ...
      log.Printf("Match. Number: %v | %v | %v | %v - Symble: %v | %v", Number_Info.Value, Number_Info.Line, Number_Info.Start_Position, Number_Info.End_Position, Symble_Info.Line, Symble_Info.Position)

      Number_Sum += Number_Info.Value
    }
  }

  return Number_Sum, nil
}
