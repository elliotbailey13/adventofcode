...
func main() {
  ...
  log.Printf("Number of left over: %v", len(Sorted_Hands))
...
func Load_Data(Line string) error {
  ...
  Hands = append(Hands, Hand{ Raw_Hand: Line_Parts[0], Bid: Hand_Bid })
...
func Convert_Hand_String_Int(Hand_Cards string) ([]int, error) {
  ...
  for _, Hand_Card := range Hand_Cards_Strings {
    switch Hand_Card {
      ...
      case "J": 
        Temp_Hand_Cards_Converted = append(Temp_Hand_Cards_Converted, 11)
