  ...
  for _, Full_Input_Line := range Full_Input {
    ...
    switch Line_Split[0] {
      case "A": // Rock
        ...
        if Line_Split[1] == "Y" { Total_Score += 1 }
        if Line_Split[1] == "Z" { Total_Score += 2 }
      case "B": // Paper
        if Line_Split[1] == "X" { Total_Score += 1 }
        if Line_Split[1] == "Y" { Total_Score += 2 }
        if Line_Split[1] == "Z" { Total_Score += 3 }
      case "C": // Scissors
        if Line_Split[1] == "X" { Total_Score += 2 }
        if Line_Split[1] == "Y" { Total_Score += 3 }
        if Line_Split[1] == "Z" { Total_Score += 1 }
      default:
